...
Then tiri will ask windows to omit client-side decorations, and also inform them that they are being tiled (which makes some windows rectangular, even if they cannot omit the decorations).
...
### Why doesn't tiri integrate Xwayland like other compositors? {#why-doesnt-tiri-integrate-xwayland-like-other-compositors}

This is an upstream niri design decision that tiri inherits. A combination of factors:

- Integrating Xwayland is quite a bit of work, as the compositor needs to implement parts of an X11 window manager.
- The X11 windowing model conflicts with the goal of having the best code for Wayland.
- There is no good global coordinate system required by X11.
...
- Tiri isn't a Big Serious Desktop Environment which Must Support All Use Cases (and is Backed By Some Corporation).

All in all, the situation works out in favor of avoiding Xwayland integration.

<sup>Upstream niri: 25.08</sup> Tiri has seamless built-in xwayland-satellite integration (inherited from niri) that by and large works as well as built-in Xwayland in other compositors, solving the hurdle of having to set it up manually.

Down the road, xwayland-satellite may well become the standard way of integrating Xwayland into new compositors, since it takes on the bulk of the annoying work and isolates the compositor from misbehaving clients.
...
You can emulate this with a script that uses the tiri IPC.
...
This is tiri's locked session background.

You can recover from this by spawning a new screen locker.
One way is to switch to a different TTY (with a shortcut like <kbd>Ctrl</kbd><kbd>Alt</kbd><kbd>F3</kbd>) and spawning a screen locker to tiri's Wayland display, e.g. `WAYLAND_DISPLAY=wayland-1 swaylock`.
...
Kanshi has its own simple configuration and communicates with tiri via IPC. You may want to launch kanshi from the tiri config.kdl e.g. `spawn-at-startup "/usr/bin/kanshi"`
...
They draw their own 1 px dark border around the window, which obscures one pixel of tiri's border.
