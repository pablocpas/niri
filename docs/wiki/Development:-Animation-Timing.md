...
The solution in tiri is a `LazyClock`, a clock that remembers one timestamp.
...
In tiri, the timestamp is cleared at the end of every event loop iteration, right before going to sleep waiting for new events.
...
The clock is shared among all animations in tiri through passing around and storing a reference-counted pointer.
