...
We implement the `org.freedesktop.a11y.KeyboardMonitor` D-Bus interface for Orca to listen and grab keyboard keys, and we expose the main tiri UI elements via [AccessKit](https://accesskit.dev).
Specifically, tiri will announce:
...
If you're shipping tiri and would like to make it work better for screen readers out of the box, consider the following changes to the default tiri config:
...
- Add some [`spawn-at-startup`](./Configuration:-Miscellaneous.md#spawn-at-startup) command that plays a sound which will indicate to users that tiri has finished loading.
- Add `spawn-at-startup "orca"` to run Orca automatically at tiri startup.
