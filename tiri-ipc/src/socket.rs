//! Helper for blocking communication over the tiri socket.
...
/// Name of the environment variable containing the tiri IPC socket path.
pub const SOCKET_PATH_ENV: &str = "TIRI_SOCKET";

/// Helper for blocking communication over the tiri socket.
///
/// This struct is used to communicate with the tiri IPC server. It handles the socket connection
...
impl Socket {
    /// Connects to the default tiri IPC socket.
    ///
    ...
    pub fn connect() -> io::Result<Self> {
        let socket_path = env::var_os(SOCKET_PATH_ENV).ok_or_else(|| {
            io::Error::new(
                ...
                format!("{SOCKET_PATH_ENV} is not set, are you running this within tiri?"),
            )
        })?;
        Self::connect_to(socket_path)
    }

    /// Connects to the tiri IPC socket at the given path.
    ...
    /// Sends a request to tiri and returns the response.
    ///
    /// Return values:
    ///
    /// * `Ok(Ok(response))`: successful [`Response`](crate::Response) from tiri
    /// * `Ok(Err(message))`: error message from tiri
    /// * `Err(error)`: error communicating with tiri
