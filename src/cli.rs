...
use tiri_ipc::{Action, OutputAction};
...
pub struct Cli {
    /// Path to config file (default: `$XDG_CONFIG_HOME/tiri/config.kdl`).
    ///
    /// This can also be set with the `TIRI_CONFIG` environment variable. If both are set, the
...
#[derive(Subcommand)]
pub enum Sub {
    /// Communicate with the running tiri instance.
    Msg {
        #[command(subcommand)]
        msg: Msg,
...
pub enum Msg {
    ...
    Output {
        /// Output name.
        ///
        /// Run `tiri msg outputs` to see the output names.
        #[arg()]
        output: String,
    ...
    EventStream,
    /// Print the version of the running tiri instance.
    Version,
    /// Request an error from the running tiri instance.
    RequestError,
