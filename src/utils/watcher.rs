...
use tiri_config::{Config, ConfigParseResult, ConfigPath};
...
pub fn setup(state: &mut State, config_path: &ConfigPath, includes: Vec<PathBuf>) {
    ...
    state
        ...
        .insert_source(
            rx,
            |event: calloop::channel::Event<Result<Config, ()>>, _, state: &mut State| match event {
...
mod tests {
    ...
    fn change_file() -> Result {
        TestPath::Explicit("tiri/config.kdl")
            .setup(|sh| sh.write_file("tiri/config.kdl", "a"))
            .assert_initial("a")
            .run(|sh, test| {
                sh.write_file("tiri/config.kdl", "b")?;
    ...
    fn overwrite_but_dont_change_file() -> Result {
        TestPath::Explicit("tiri/config.kdl")
            ...
            .assert_initial("a")
            .run(|sh, test| {
                sh.write_file("tiri/config.kdl", "a")?;
    ...
    fn touch_file() -> Result {
        TestPath::Explicit("tiri/config.kdl")
            ...
            .assert_initial("a")
            .run(|sh, test| {
                cmd!(sh, "touch tiri/config.kdl").run()?;
                test.assert_changed_to("a");
    ...
    fn change_linked_file() -> Result {
        TestPath::Explicit("tiri/config.kdl")
            .setup(|sh| {
                sh.write_file("tiri/config2.kdl", "a")?;
                cmd!(sh, "ln -sf config2.kdl tiri/config.kdl").run()
            })
            .assert_initial("a")
            .run(|sh, test| {
                sh.write_file("tiri/config2.kdl", "b")?;
                test.assert_changed_to("b");
    ...
    fn change_file_in_linked_dir() -> Result {
        TestPath::Explicit("tiri/config.kdl")
            .setup(|sh| {
                sh.write_file("tiri2/config.kdl", "a")?;
                cmd!(sh, "ln -s tiri2 tiri").run()
            })
            .assert_initial("a")
            .run(|sh, test| {
                sh.write_file("tiri2/config.kdl", "b")?;
                test.assert_changed_to("b");
    ...
    fn remove_file() -> Result {
        TestPath::Explicit("tiri/config.kdl")
            ...
            .assert_initial("a")
            .run(|sh, test| {
                sh.remove_path("tiri/config.kdl")?;
                test.assert_unchanged();
    ...
    fn swap_dir_link() -> Result {
        TestPath::Explicit("tiri/config.kdl")
            ...
            .assert_initial("a")
            .run(|sh, test| {
                sh.write_file("tiri3/config.kdl", "b")?;
                sh.remove_path("tiri")?;
                cmd!(sh, "ln -s tiri3 tiri").run()?;
                test.assert_changed_to("b");
    ...
    fn change_included_file() -> Result {
        TestPath::Explicit("tiri/config.kdl")
            .setup(|sh| {
                sh.write_file("tiri/config.kdl", "include \"colors.kdl\"")?;
                sh.write_file("tiri/colors.kdl", "// Colors")
            })
            .assert_initial("include \"colors.kdl\"")
            .run(|sh, test| {
                sh.write_file("tiri/colors.kdl", "// Updated colors")?;
    ...
    fn remove_included_file() -> Result {
        TestPath::Explicit("tiri/config.kdl")
            ...
            .assert_initial("include \"colors.kdl\"")
            .run(|sh, test| {
                sh.remove_path("tiri/colors.kdl")?;
                test.assert_changed_to("include \"colors.kdl\"");
    ...
    fn nested_includes() -> Result {
        TestPath::Explicit("tiri/config.kdl")
            .setup(|sh| {
                sh.write_file("tiri/config.kdl", "include \"a.kdl\"")?;
                sh.write_file("tiri/a.kdl", "include \"b.kdl\"")?;
                sh.write_file("tiri/b.kdl", "// b content")
            })
            .assert_initial("include \"a.kdl\"")
            .run(|sh, test| {
                sh.write_file("tiri/b.kdl", "// updated b")?;
    ...
    fn broken_include_still_gets_watched() -> Result {
        TestPath::Explicit("tiri/config.kdl")
            .setup(|sh| {
                sh.write_file("tiri/config.kdl", "include \"colors.kdl\"")?;
                sh.write_file("tiri/colors.kdl", "broken")
            })
            .assert_initial("include \"colors.kdl\"")
            .run(|sh, test| {
                sh.write_file("tiri/colors.kdl", "// Fixed")?;
                test.assert_changed_to("include \"colors.kdl\"");
    ...
    fn swap_just_link() -> Result {
        TestPath::Explicit("tiri/config.kdl")
            .setup_any(|sh| {
                let dir = sh.current_dir().join("tiri");
                ...
                cmd!(sh, "ln -s config2.kdl tiri/config.kdl").run()?;

                Ok(())
            })
            .assert_initial("a")
            .run(|sh, test| {
                cmd!(sh, "ln -sf config3.kdl tiri/config.kdl").run()?;
                test.assert_changed_to("b");
    ...
    fn swap_many_regular() -> Result {
        TestPath::Regular {
            user_path: "user-tiri/config.kdl",
            system_path: "system-tiri/config.kdl",
        }
        .setup(|sh| sh.write_file("system-tiri/config.kdl", "system config"))
        .assert_initial("system config")
        .run(|sh, test| {
            sh.write_file("user-tiri/config.kdl", "user config")?;
            test.assert_changed_to("user config");

            cmd!(sh, "touch system-tiri/config.kdl").run()?;
            test.assert_unchanged();

            sh.remove_path("system-tiri")?;
            test.assert_unchanged();

            sh.write_file("system-tiri/config.kdl", "new system config")?;
            test.assert_unchanged();

            sh.remove_path("user-tiri")?;
            test.assert_changed_to("new system config");

            sh.write_file("system-tiri/config.kdl", "updated system config")?;
            test.assert_changed_to("updated system config");

            sh.write_file("user-tiri/config.kdl", "new user config")?;
    ...
    fn swap_many_links_regular_like_nix() -> Result {
        ...
        .setup_any(|sh| {
            ...
            sh.create_dir("user-tiri")?;
            sh.create_dir("system-tiri")?;
        ...
        .run(|sh, test| {
            ...
            cmd!(sh, "ln -s {store}/gen1 user-tiri/config.kdl").run()?;
            test.assert_changed_to("gen 1");

            cmd!(sh, "ln -s {store}/gen2 system-tiri/config.kdl").run()?;
            test.assert_unchanged();

            cmd!(sh, "unlink user-tiri/config.kdl").run()?;
            test.assert_changed_to("gen 2");

            cmd!(sh, "ln -s {store}/gen3 user-tiri/config.kdl").run()?;
            test.assert_changed_to("gen 3");

            cmd!(sh, "ln -sf {store}/gen1 system-tiri/config.kdl").run()?;
            test.assert_unchanged();

            cmd!(sh, "unlink system-tiri/config.kdl").run()?;
            test.assert_unchanged();

            cmd!(sh, "ln -s {store}/gen1 system-tiri/config.kdl").run()?;
            test.assert_unchanged();
